...
files.Rnw    := $(call filter-buildable,Rnw)
...
default_files.Rnw    := $(call filter-default,Rnw)
...
all_files_source_gen     := $(call concat-files,rst rhs Rnw,all)
...
default_files_source_gen := $(call concat-files,rhs lhs Rnw,default)
default_files_scripts    := $(call concat-files,tex.sh tex.pl tex.py,default)

files_source             := $(call concat-files,tex)
files_source_gen         := $(call concat-files,rst lhs Rnw)
...
all_stems.Rnw        := $(call get-stems,Rnw,all)
...
default_stems.Rnw    := $(call get-stems,Rnw,default)
...
all_stems_source_gen     := $(call concat-stems,rst lhs Rnw,all)
...
default_stems_source_gen := $(call concat-stems,rst lhs Rnw,default)
...
stems_source_gen         := $(call concat-stems,rst lhs Rnw)
...
real_goals := $(call get-default,$(filter-out _includes,$(MAKECMDGOALS)),all)
...
# TODO: sort by category, comment what files are which
rm_ext := \
	log *.log aux $(pre_pdf_extensions) pdf blg bbl bcf out nav snm toc lof lot lol pfg \
...
define get-R-inputs
$(SED) -n \
-e 's!processing file: \(.*\)!$(2) : \1! p' \
'$1' | $(SORT) | $(UNIQ)
endef

define get-R-outputs
$(SED) -n \
-e 's/output file: \(.*\)/\1/ p' \
'$1' | $(SORT) | $(UNIQ)
endef
...
# Outputs all bibliography files used by biblatex to stdout. Arg 1 is the
# .bcf, arg 2 is a list of targets for each dependency found.
#
# The script parses the .bcf file for data sources, extracts .bib filenames
# from it and produces a list of space-delimited .bib filenames, which is what
# the make dep file expects to see.
#
# Note that we give kpsewhich a bogus argument so that a failure of sed to
# produce output will not cause an error.
#
# $(call get-biblatex-bibs,<bcf file>,<targets>)
define get-biblatex-bibs
$(SED) \
`# Parses a .bcf file to find datasource tag` \
-e '/^[[:space:]]*<bcf:datasource[[:space:]]/!d' \
...
%.bbl: %.auxbbl.make
	$(QUIET)\
	$(if $(filter %.bib,$^),\
		if $(call test-exists,$*.bcf); then \
			$(call echo-build,$(strip $(filter %.bib,$?) $*.bcf),$@); \
			$(call set-run-reason,dependencies of $@ changed); \
			$(call run-biber,$*); \
		else \
			$(call echo-build,$(strip $(filter %.bib,$?) $*.aux),$@); \
			$(call set-run-reason,dependencies of $@ changed); \
			$(call run-bibtex,$*); \
		fi; \
		$(TOUCH) $@.cookie; \
	...
	$(if $(filter %.bib,$^),\
		if $(call test-exists,$*.bcf); then \
			$(call echo-build,** annotated extra bibtex ** $(filter %.bib,$?) $*.bcf,$@); \
			$(call run-biber,$*); \
		else \
...
# Create an acronym file from a glossary input formatted for xindy
%.acr: %.acn %.tex %.xdy
	$(QUIET)$(call echo-build,$<,$@)
	$(QUIET)$(call run-xindy,$<,$@,$*,$*.alg)
...
# create dependencies, make output and .tex from Rnw documents
# begin by setting up the R script (there's probably a better way to do this)
# run the R script
# Filter the log files (.tex.make) to produce .fls documents
# so that the dependency generation sed scripts correctly identify
# new dependencies (generated graphics, tex files)
# and reverse dependencies (Rnw and R files)
# Specifically, this enables the generated files to be 'cleaned'
#
# Then explicitly add the dependencies to the .tex.d file
#
# TODO: colorize R log
%.tex.make %.tex:: %.Rnw
	...
	$(QUIET)$(RSCRIPT) \
		...
		-e 'real_plot = knit_hooks$$get( "plot" )' \
		-e 'knit_hooks$$set(' \
		-e '  plot = function( x , options ) {' \
		-e '    message( "output file: " , x )' \
		-e '    real_plot( x , options )' \
		-e '  } ,' \
		-e '  file.chunk = function( before , options , envir ) {' \
		-e '    if( !before ) {' \
		-e '      message( "processing file: " , options[["file.chunk"]] )' \
		-e '      read_chunk( options[["file.chunk"]] )' \
		-e '    }' \
		-e '  }' \
		-e ')' \
		-e 'opts_hooks$$set( ' \
		-e '  child = function(options) {' \
		-e '    lapply(options$$child , function(x) {message( "processing file: " , x)} )' \
		-e '    return(options)' \
		-e '  } ,' \
		-e '  cache = function(options) {' \
		-e '    options$$cache.path <- paste("cache" , current_input() , "" , sep = "/")' \
		-e '    return(options)' \
		-e '  }' \
		-e ')' \
...
%.$(build_target_extension).1st.make %.d %.aux %.aux.make %.fls: %.tex
	...
	$(call test-exists,$*.bcf) && $(call get-biblatex-bibs,$*.bcf,$(addprefix $*.,bbl aux aux.make bcf)) >> $*.d; \
	$(call test-exists,$*.aux.make) && $(call get-bibs,$*.aux.make,$(addprefix $*.,bbl aux aux.make)) >> $*.d; \
